...
def test_lora_initialization():
    ...
    assert lora_layer.A.shape == (
        original_layer.weight.size(0),
        r,
    ), "Matrix A is not the correct size."
    assert lora_layer.B.shape == (
        r,
        original_layer.weight.size(1),
    ), "Matrix B is not the correct size."

    # Check that the scaling factor is computed correctly
    expected_scaling = alpha / r
    assert (
        lora_layer.scaling == expected_scaling
    ), f"Scaling factor is incorrect. Expected {expected_scaling}, got {lora_layer.scaling}."

    # Check that matrix B is initialized to zeros
    assert torch.equal(
        lora_layer.B, torch.zeros_like(lora_layer.B)
    ), "Matrix B is not initialized to zeros."

    # Check that matrix A is not zero (kaiming uniform initialization)
    assert not torch.equal(
        lora_layer.A, torch.zeros_like(lora_layer.A)
    ), "Matrix A is incorrectly initialized to zeros."
...
def test_lora_forward_pass():
    ...
    assert (
        lora_output.shape == original_output.shape
    ), "LoRA output shape does not match original layer output shape."

    # Check that the output is different from the original output (LoRA applied)
    assert not torch.equal(
        lora_output, original_output
    ), "LoRA output should differ from the original layer output."
    ...
    assert torch.allclose(
        lora_output, expected_output, atol=1e-4
    ), "LoRA output doesn't match the expected output."
...
def test_inject_lora():
    ...
    assert (
        num_lora_layers == 72
    ), "Not all linear attention layers were substituted by LoRA layers"
...
def test_soft_prompt_initialization():
    ...
    assert soft_prompt.soft_prompt.shape == (
        prompt_length,
        model_hidden_size,
    ...
    assert not torch.equal(
        soft_prompt.soft_prompt, torch.zeros_like(soft_prompt.soft_prompt)
...
def test_soft_prompt_forward_pass():
    ...
    assert (
        combined_embeddings.shape[1] == input_embeddings.shape[1] + prompt_length
...
def test_soft_prompt_backpropagation():
    ...
    assert (
        soft_prompt.soft_prompt.grad is not None
    ), "Gradients were not backpropagated through the soft prompts."
    assert torch.any(
        soft_prompt.soft_prompt.grad != 0
    ), "Gradients for soft prompt parameters are all zero."
